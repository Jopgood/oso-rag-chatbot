...
* A [Google AI Studio API key](https://ai.google.dev/) to generate embeddings and chatbot responses.
    * **NOTE:** The requests to Google Generative AI will incur charges based on your usage.
...
* `GOOGLE_API_KEY`: (required) The API key used for Google Generative AI operations. You can get a key from [Google AI Studio](https://ai.google.dev/) after creating an account. 
...
* `DEBUG`: (optional) Used to emit various debugging information. Unset by default (no debug logging). Can be set to any combination of the following as a comma-delimited string:
    ...
    * `embedding`: Emit the vector embeddings generated by Google.
